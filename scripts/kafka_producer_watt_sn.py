...
if __name__ == '__main__':
    ...
    while True:
        ...
        for zone in ZONES:
            producer.send('watt_conso_raw', gen_conso(zone, h))
            producer.send('watt_solaire_raw', gen_solaire(zone, h))
        producer.flush()
        print(f"[{datetime.utcnow().strftime('%H:%M:%S')}] Batch envoyé — 7 zones")
        time.sleep(5)